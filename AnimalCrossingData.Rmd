...
The data set we foung on Tidy Tuesday has 4 different sets within it: Items, Villagers, Critic reviews, and User reviews. In this blog we wil breaking up into different sections to explore each data set to answer some questions we may have and what we can discover from these datasets.
...
Here we will filter out the table to focus on furniture sets:
```{r}
...
View(cheap_furniture_items)
View(expensive_furniture_items)

mycolors = c(brewer.pal(name="Accent", n = 3), brewer.pal(name="Dark2", n = 8), brewer.pal(name="Set2", n = 7))
...
We can also use the user reviews (0-10) and critic ratings (0-100) to find the average rating.
