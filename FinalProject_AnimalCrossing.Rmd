...
The data set we found on TidyTuesday has 4 differnt sets within it: Items, Villagers, Critic reviews, and User reviews. In this blog, we wil breaking up into differnt sections to explore each data set to answer some questions we may have and what we can discover from these datasets.
...
Here we will filter out the table to focus on furniture sets and then try to see which furniture sets are the cheapest and most expensive to purchase in the game.
...
ggplot(data = cheap_furniture_items, aes(x = fct_reorder(furniture_set, mean_buy_value), y = mean_buy_value)) +
  ...
  coord_flip() +
  labs(title = "5 Cheapest Furniture Sets in Animal Crossing", x = "Furniture Set", y ="Average Buy Value")

```

Looks like the average price of an item in the cheapest sets range from 180 to 350 bells! Not too bad for the price, though they probably don't look that good...
...
ggplot(data = expensive_furniture_items, aes(x = fct_reorder(furniture_set, mean_buy_value), y = mean_buy_value)) +
  geom_col(aes(fill = furniture_set)) +
  coord_flip() +
  labs(title = "5 Most Expensive Furniture Sets in Animal Crossing", x = "Furniture Set", y ="Average Buy Value")
```
...
Villagers in animal crossing are...animals and there are many different types of animals in the game, 35 differnet species to be exact! Each villager has their own personality which can be categorized into 8 different types (4 female, 4 male): Normal, Peppy, Snooty, Uchi (sisterly), Cranky, Jock, Lazy, and Smug.

Let's take a look and see if the differnt personality types are evenly distributed amongst villagers or if there are more common personalities compared to others.
...
Based off the table, it looks like there doesn't seem to be an even distribution of personalities. Let's take a look at a graph to get a better visualization:
...
When you look at the two graphs you can see how there is always the most common trait, then 2 common traits which arent that far off (in terms of numbers) from the most common trait, and then the least common trait falls behind by quite a bit.

Therfore it looks like the chances of meeting a lazy male villager or normal female villager are higher than meeting a smug male villager or uchi female villager.
...
Intersting, it seems as though the personality types within the cats is not evenly distributed. Looks like the most common personality trait of cats is snooty and peppy and the lease common traits are the smug and uchi personalities. I wonder if this is the same for the other animals?
...
Based off of these graphs, it looks like the type of personality each villager has is based off of what type of animal they are and is either completly random or tries to relate to the sterotype that an animal typically has in real life. For example, the cat sterotype of being more snooty or peppy, or the rabbit sterotype of being more peppy/energetic.
...
Looking at these grpahs, it also appears as though users have some good things to say about the game but also some more negative things to say compared to the critics.

### Comparing the Ratings

Now that we have a good idea of what kind of reviews the users and critics gave, let's take a look at the actual grades that were given.
...
Animal Crossing: New Horizons is a step up from any other edition of the game by far. In this edition, players have the ability to terraform and shpape the island in any way that they want, they have access to more furniture, can craft custom recipes for items, and overall have a lot of freedom to do what they want! With that said, these data sets are a sample of what the gameplay is like. As seen from the long list of items and how cheap or expensive furniturei is that players can either buy or sell, or from the long list of diffenent villagers/personalities that players can meet, there is a lot to do in the game!

However, as the reviews show...while critics love the game it appears as though many users do not. The graphs that we created show the positive sentiment in many reviews, but our comparison of the average ratings showed that despite the positive sentiment in user reviews, the average grade says otherwise (The reason for this has already been explained). On the other hand, we showed that there was a lot of positive sentiment in the critic reviews and the grade followed through :).Hopefully, the analysis from this blog made you interseted in playing Animal Crossing but if not, hopefully it kept you a bit interested in the game.
